-- Ricrea le views con SECURITY INVOKER invece di SECURITY DEFINER
-- per rispettare le RLS policies dell'utente che le interroga

DROP VIEW IF EXISTS dealers_with_stats;
DROP VIEW IF EXISTS commerciali_with_stats;

-- View ottimizzata per dealers con statistiche aggregate (SECURITY INVOKER)
CREATE VIEW dealers_with_stats 
...
-- View ottimizzata per commerciali con statistiche aggregate (SECURITY INVOKER)
CREATE VIEW commerciali_with_stats
WITH (security_invoker = true) AS
SELECT 
  p.id,
  p.display_name,
